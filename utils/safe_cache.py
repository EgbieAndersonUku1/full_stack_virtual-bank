...
def get_cache_or_set(key: str, 
                     value_or_func: Union[Any, Callable[[], Any]], 
                     ttl: int = 300,
                     retry_attempts: int=5) -> Any:
    """
    ...
    Args:
        ...
        retry_attempts: The number of retry attempts the function can try before falling back to default save
    ...
    with cache_lock(key) as locked:
        ...
        if locked:
            ...
            if value is None:
                ...
                logger.debug(f"setting to cache using the key = {key} and retreiving cache value")

                cache.set(key, value, ttl)

                return value

    # Another worker/process is likely populating the cache
    # Retry reading from cache for a short period
    logger.debug(f"Attempting to retreive cache via retries. Retries ={retry_attempts}")
    value = get_cache_with_retry(key, retries=retry_attempts)

    if value is not None:
        logger.info("Successfully retrieved cached with retries")
        return value

  
    # Fall back and Compute directly so caller never gets None unexpectedly
    logger.debug(f"Retries failed, falling back to save data using the default save cache method. Set TTl={ttl}")
    value = (
        value_or_func()
