...
def has_permissions_to_view_page(func):
    @wraps(func)
    def wrapper(request, *args, **kwargs):
        try:
            if request.user and not(request.user.is_superuser or request.user.is_staff or request.user.is_admin):
                return redirect("dashboard")
            return func(request, *args, **kwargs)
        except AttributeError:
             return func(request, *args, **kwargs)
...
def go_to_staff_page(func):
    ...
    def wrapper(request, *args, **kwargs):
        try:
            if request.user and (request.user.is_superuser or request.user.is_staff or request.user.is_admin):
